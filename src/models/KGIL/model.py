...
# This is a replacement for the torch_scatter.scatter_sum function
# Resolvs issues with dependency on torch_scatter
def scatter_sum(src: torch.Tensor, index: torch.LongTensor, dim_size: int = None) -> torch.Tensor:
    """
    Replacement for torch_scatter.scatter_sum.
    src:    (N, D)
    index:  (N,)  long tensor of row‐ids
    returns (dim_size, D)
    """
    if dim_size is None:
        dim_size = int(index.max()) + 1
    out = torch.zeros(dim_size, src.size(1), dtype=src.dtype, device=src.device)
    # index_add_ accumulates src[i] into out[index[i]]
    return out.index_add_(0, index, src)


# Updated by Jonas Limniatis
# This is a replacement for the torch_scatter.scatter_mean function
# Resolvs issues with dependency on torch_scatter   
def scatter_mean(src: torch.Tensor, index: torch.LongTensor, dim_size: int = None) -> torch.Tensor:
    """
    Replacement for torch_scatter.scatter_mean.
    """
    summed = scatter_sum(src, index, dim_size)
    counts = torch.bincount(index, minlength=summed.size(0)).unsqueeze(1).to(src.dtype)
    counts = counts.clamp(min=1)   # avoid div‐zero
...
class Recommender(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat, device, augmenter=None):
